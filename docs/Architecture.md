...
- **Bounded paced sends**: the client TUN reader and the server's per-client writers send through a space-checked fast path (`send_one_datagram` in `stream.rs`): when `datagram_send_buffer_space()` fits the datagram — guaranteed not to evict older queued datagrams — the non-waiting `send_datagram` is used, skipping the wait-future setup per datagram; only under backpressure does the sender await `send_datagram_wait`, applying QUIC backpressure directly. Each connection has exactly one datagram-producing task, so the space check cannot race. The server TUN reader feeds a bounded queue per client and each independent writer drains it in `recv_many` batches, preserving client isolation. Unguarded non-waiting sends are never used — they silently evict older queued datagrams while reporting success.
