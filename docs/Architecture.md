...
- **Transport**: both endpoints build the identical fixed QUIC transport config from `build_quic_transport_config` (`src/transport/mod.rs`): Cubic congestion control, 8 MB connection/stream receive and send windows (`QUIC_WINDOW_SIZE`), a 4 MB datagram receive buffer, a 256 KiB datagram send buffer, an ACK-frequency threshold of one ACK per 16 packets (`QUIC_ACK_ELICITING_THRESHOLD` — each IP packet is one QUIC packet, so at >100k packets/s the default ACK-every-other-packet is a first-order CPU cost on both ends), 15 s keep-alive, and 30 s idle timeout. Cubic replaced paced BBRv3 on measurement: through the tunnel with one inner TCP flow, on a LAN path and on emulated 40 ms paths (clean, 0.5% random loss, and a 200 Mbit/s bottleneck with a ~1.4 BDP queue), Cubic moved 11–51% more in every case and filled the bottleneck (89% vs 75–80%). BBRv3's one gain was a shorter queue at that bottleneck in the server→client direction (80 vs 148 ms loaded RTT; barely any client→server, and more loaded latency than Cubic on the LAN). Its premise for a tunnel — that the outer controller's loss response multiplies the inner TCP's — did not show: under random loss the inner TCP's own response caps the flow whichever controller runs outside. BBRv3 stays selectable with `--congestion-control bbr3`. Sends wait for room in the bounded queue (via a space-checked fast path) so QUIC's pacer regulates TUN reads instead of the non-blocking API silently evicting old datagrams. QUIC datagrams are enabled — they carry the IP data path. Because nothing differs between the sides, there is no transport dictation in the handshake and no reconnect-to-upgrade step.
  - **Congestion-control override**: the one transport setting that is not a compile-time constant. `server start` / `client start` accept `--congestion-control <cubic|bbr3|new-reno>` and `--congestion-initial-window <BYTES>` (`CongestionConfig` in `src/transport/mod.rs`), so an alternative controller or initial window can be measured on a real path without a rebuild. They are intentionally CLI-only — never config-file keys — and are latched process-wide in a `OnceLock` before any endpoint is created, so every transport config the process builds (reconnects, the relay probe, additional endpoints) uses the same settings. A global rather than a threaded parameter because `build_quic_transport_config` is also reached from the iOS/Windows FFI entry points, which parse no CLI arguments and should not carry a testing knob in their API. Congestion control is sender-local and unnegotiated, so the two ends may legitimately run different settings. The initial window is bounded by `CONGESTION_INITIAL_WINDOW_MIN`/`_MAX` (two `QUIC_INITIAL_MTU` packets up to `QUIC_WINDOW_SIZE`) at parse time, because noq applies the value verbatim without clamping; every other controller parameter keeps its noq default.
...
- **Bounded paced batch sends**: the client TUN reader (one call per TUN read, i.e. every segment of a GSO super-frame) and the server's per-client writers (one call per `recv_many` batch) queue datagrams through `send_datagram_batch` in `stream.rs`. One `datagram_send_buffer_space()` read sizes the longest prefix that fits, and one `send_many_datagrams` queues it; when not even the next datagram fits, that datagram awaits `send_datagram_wait`, applying QUIC backpressure directly. The connection's state lock is shared with its driver, which holds it while it encrypts and sends, so taking it once per run instead of several times per datagram keeps the producer from serializing against the driver (`max_datagram_size()` is likewise read once per TUN packet). `send_many_datagrams` evicts older datagrams to make room, so it is only ever handed a prefix the space check guarantees fits: each connection has exactly one datagram-producing task and the driver only frees space, so the check cannot race. The server TUN reader feeds a bounded queue per client and each independent writer drains it, preserving client isolation. Unguarded non-waiting sends are never used — they silently evict older queued datagrams while reporting success.
- **Batched datagram reads**: the client and server receive paths drain up to `DATAGRAM_READ_BATCH` (64) datagrams per `read_many_datagrams` call, one connection-lock acquisition per burst instead of per packet.
...
- **Datagram arena**: per-datagram `Bytes` are split off a long-lived 64 KB `BytesMut` (`copy_packet_to_arena`), so the allocator is hit once per arena chunk instead of once per datagram. Inbound datagrams arrive as owned `Bytes` from `read_many_datagrams` and cross the TUN-writer channel with no copy.
- **Zero-copy sends**: `Bytes` flow from the arena to the QUIC datagram send without copying.
- **Full-size UDP socket buffers**: iroh asks for 7 MiB socket buffers, which Linux silently caps at `net.core.rmem_max`/`wmem_max` (about 208 KiB by default); a buffer that small overflows under bulk traffic (UDP `RcvbufErrors`), and the loss lands on the congestion controller and the inner TCP flow. The netwatch fork (see `[patch.crates-io]` in `Cargo.toml`) sets the buffers with `SO_RCVBUFFORCE`/`SO_SNDBUFFORCE` first, which bypass the cap for a process with `CAP_NET_ADMIN` — ezvpn runs as root for the TUN device — and falls back to the capped options otherwise. It sits in netwatch's socket bind, so the sockets it rebinds after network changes keep the full size too.
- **Accurate RTT samples on fast paths**: the noq fork stamps each received batch as it is read (upstream stamps a whole receive cycle with the instant its driver poll began) and makes BBRv3 (`--congestion-control bbr3`) skip a non-positive RTT sample. Upstream, a cycle that outlasted a round trip could stamp an ACK before the packet it acknowledges was sent; the saturating `Instant` subtraction read that as a zero RTT, which the 10 s min-RTT filter held, zeroing the BDP estimate and leaving the congestion window to the ACK-aggregation term alone. On a sub-millisecond LAN path this swung the window between tens of kilobytes and hundreds of megabytes.
