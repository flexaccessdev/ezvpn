...
use iroh::{Endpoint, EndpointId, RelayUrl, Watcher};
...
pub struct VpnServerStats {
    ...
    /// Packets dropped because they exceeded the connection's current max
    /// datagram size (path-MTU capped). WireGuard-style: the inner flow adapts
    /// as DPLPMTUD raises the path.
    pub packets_dropped_too_large: AtomicU64,
}
...
impl VpnServer {
    ...
    ///
    /// With custom relays the shared home-relay failover
    /// (`flexaccess_iroh::relay_failover`) runs alongside the accept loop and
    /// keeps the server dialable: `relay_config` is what it moves between, and
    /// `relays_left_out` are the configured relays the endpoint was bound
    /// without because they failed the startup probe, which it puts back once
    /// they are connectable. With the default relays it is pending forever.
    pub async fn run(
        mut self,
        endpoint: Endpoint,
        ...
        let tun_endpoint = endpoint.clone();
        let tun_reader_handle = tokio::spawn(async move {
            if let Err(e) = server_tun.run_tun_reader(tun_reader, &tun_endpoint).await {
        ...
        // address lookup is off, clients dial by relay hint), and if that
        // relay is lost for a minute without iroh re-homing on its own, the
        // failover moves the endpoint onto another configured relay in place.
        // Nothing is torn down: the identity, the direct paths, the
        // established connections and the TUN device all stay. Relays the
        // startup probe could not connect (the endpoint was bound without
        // them) are put back by the same failover once they are. With the
        // default relays the failover future is pending forever.
        tokio::select! {
            () = server.accept_loop(&endpoint, &tun_write_tx) => {
                log::info!("Endpoint closed, shutting down");
            }
            () = fail_over_home_relay(&endpoint, relay_config, relays_left_out) => {}
        }
    ...
    async fn handle_connection_inner(
        ...
        // The tunnel MTU is the fixed protocol constant VPN_MTU. Each IP packet
        // maps to one unreliable QUIC datagram; a packet exceeding the live
        // datagram size is dropped rather than fragmented (the inner flow adapts).
        ...
        let response = match (assigned_ip, assigned_ip6) {
            ...
            (Some(ip4), None) => {
                let ip_pool = self.ip_pool.as_ref().unwrap().read().await;
                VpnHandshakeResponse::accepted(ip4, host_net4(ip_pool.server_ip()), ip_pool.server_ip())
            }
            // IPv6-only
            (None, Some(ip6)) => {
                let ip6_pool = self.ip6_pool.as_ref().unwrap().read().await;
        ...
        // Spawn dedicated control-stream writer task that owns the stream send
        // half and writes each queued `ServerAddrs` frame. Returns errors via
        // the oneshot channel. At least one IP must be set at this point.
        ...
        let writer_handle = tokio::spawn(async move {
            ...
            let error = loop {
                ...
                if let Err(reason) = write_frames(&mut send, &mut batch).await {
                    log::warn!(
                        "Failed to write control frames to {}: {}",
                        writer_client_id, reason
                    );
                    break Some(reason.to_string());
                }
            };
            log::trace!("Control writer task for {} exiting", writer_client_id);
    ...
    /// Run the TUN reader - reads packets from TUN and sends each to the
    ...
    ///
    /// `endpoint`'s local UDP port set is the self-encapsulation filter; the
    /// endpoint is bound once for the life of `run` (a relay failover moves
    /// its home relay, not its sockets), so the set is read once.
    async fn run_tun_reader(
        ...
        let local_iroh_udp_ports = collect_local_iroh_udp_ports(endpoint);
